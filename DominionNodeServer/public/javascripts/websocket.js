...
function sendBuildingRequest(buildingType, x, y){
    console.debug("Sending building request of type "+buildingType+" "+x+","+y);
    socket.emit('building request', {username: thisClientPlayerName, buildingType: buildingType, x: x, y: y});
}

function sendBuildUnitRequest(type, owner, x, y){
    console.info("sending build unit request of type "+type+" for "+owner);
    socket.emit('build unit request', {username: owner, type: type, x: x, y: y});
}
...
socket.on('world_objects', function (data) {
    ...
    for(i = 0; i < objs.length; i++){
        var world_object = loadWorldObjectSprite(objs[i].name,objs[i].x,objs[i].y, objs[i].name, objs[i].owner, objs[i].resource_left, objs[i].type);
        worldObjects.push(world_object);
    }
});

socket.on('new building', function(data){
    var world_object = loadWorldObjectSprite(data.name, data.x,data.y, data.name, data.owner, data.resource_left, data.type);
        worldObjects.push(world_object);
});
...
socket.on('server push', function(data){
  ...
  //     console.log("SERVER PUSH: "+data.users.length+" users updated");
      ...
      //     console.log("Updating "+username+" position to "+x+","+y+" direction = "+dir);
       ...
       console.debug("Received info about "+npcs.length+" npcs");
       for(var m = 0; m < npcs.length; m++){
           console.debug("Received npc info: "+npcs[m].action+" at" + npcs[m].x+", "+npcs[m].y);
       }
